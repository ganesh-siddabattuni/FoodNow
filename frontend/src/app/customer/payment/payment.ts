...
export class PaymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.deliveryAddressForm = this.fb.group({
      ...
      pinCode: [
        '500001',
        [Validators.required, Validators.pattern(/^[1-9]\d{5}$/)],
      ],
    });
    ...
    this.cartService.getCart().subscribe({
      ...
      error: () =>
        this.notificationService.error('Could not load your cart summary.'),
    });
  }
  ...
  downloadReceipt(): void {
    ...
    import('html2canvas')
      ...
      .then((canvas) => {
        ...
        link.download = `FoodNow-Receipt-${
          this.receiptData()?.orderNumber || Date.now()
        }.png`;
        link.href = imgData;
        link.click();
      })
...
`;
    ...
    receipt.items.forEach((item) => {
      receiptText += `${item.quantity}x ${item.foodItem.name} - ₹${(
  ...
  onPlaceOrder(): void {
    if (!this.isPaymentValid()) {
      this.notificationService.error(
        'Please complete your delivery and payment details correctly.'
      );
    ...
    const receiptData: ReceiptData = {
      ...
      customerName:
        this.selectedPaymentMethod() === 'card'
          ? this.cardForm.get('cardHolder')?.value || 'Customer'
          : 'Customer',
    ...
    this.orderService.placeOrder(addressPayload).subscribe({
      next: () => {
        ...
        if (this.selectedPaymentMethod() === 'wallet') {
          this.mockWalletBalance.update(
            (balance) => balance - receiptData.finalAmount
          );
        }
      ...
      error: () =>
        this.notificationService.error('Could not place your order.'),
    });
  }
}
