...
export class AdminPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      if (
        section === 'restaurants' &&
        !this.state.sortConfig()['restaurants']?.column
      ) {
        this.sortData('name'); // Default sort by restaurant name
      }

      //PENDING APPLICATIONS
      if (
        section === 'applications' &&
        !this.state.sortConfig()['applications']?.column
      ) {
        this.sortData('restaurantName'); // Default sort by restaurant name
      }

      //ALL USERS
      if (section === 'users' && !this.state.sortConfig()['users']?.column) {
        this.sortData('name'); // Default sort by user name
      }
    });
  }
  ...
  approve(app: PendingApplication): void {
    this.adminService.approveApplication(app.id).subscribe({
      ...
      error: () =>
        this.notificationService.error('Failed to approve application.'),
    });
  }
  ...
  reject(app: PendingApplication): void {
    ...
    if (reason && reason.trim()) {
      this.adminService.rejectApplication(app.id, { reason }).subscribe({
        ...
        error: () =>
          this.notificationService.error('Failed to reject application.'),
      });
    }
  }
    ...
    this.adminService.createDeliveryAgent(this.newAgentForm).subscribe({
      ...
      error: (err) =>
        this.notificationService.error(
          err.error?.message || 'Failed to create agent.'
        ),
    });
  }
  ...
  sortedApplications = this.createSorter(
  ...
  sortedDeliveryAgents = this.createSorter(
    this.state.deliveryAgents,
    'delivery'
  );
}
